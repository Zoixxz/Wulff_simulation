...
def createConfigNorm(G, N, m, p):
    ...
    interior_vertices = set()  # set of sets of edges (tuples with coordinates)
    boundary_vertices = set()  # set of sets of edges (tuples with coordinates)
    for v in V:
                ...
                if(graphDistanceOne(w) == m + 1):  # boundary edge
                    if(G.are_connected(itaux.coord_to_array(N, v),
                        ...
                        e = G.get_eid(itaux.coord_to_array(N, v),
                                      itaux.coord_to_array(N, w))
                        G.delete_edges([e])
                    boundary_vertices.add(w)
    return G, V, interior_vertices, boundary_vertices
