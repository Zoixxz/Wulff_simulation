import ig_percoSimIterationAux_runtime as itaux
...
def createConfigNorm(G, N, m):
    ...
    # remove line diagonally across 1-norm ball leaving two edges at each end
    ...
    for i in range(m - 2):
        e_c = ((0, i), (0, i + 1))
        e_cm = ((0, -i), (0, -i - 1))
        e_coord = frozenset(((0, i), (0, i + 1)))
        e_coord_minus = frozenset(((0, -i), (0, -i - 1)))
        interior_edges.remove(e_coord)
        interior_edges.remove(e_coord_minus)
        boundary_edges.append(frozenset(e_c))
        boundary_edges.append(frozenset(e_cm))
        e0 = G.get_eid(itaux.coord_to_array(N, e_c[0]),
                       itaux.coord_to_array(N, e_c[1]))
        e1 = G.get_eid(itaux.coord_to_array(N, e_cm[0]),
                       itaux.coord_to_array(N, e_cm[1]))
