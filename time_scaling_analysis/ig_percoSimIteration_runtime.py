...
def wulffIteration_edges(G, boundary_edges, interior_edges, p, N):
    ...
    e_interior_frozen = random.choice(interior_edges)  # e_interior is a frozen set
    ...
    for i in [0, 1]:
        try:
            edges_boundary_updated.remove(frozenset((e_boundary[i], e_boundary[1 - i])))
        except ValueError:
            pass
    ...
    for i in [0, 1]:
        try:
            edges_interior_updated.remove(frozenset((e_interior[i], e_interior[1 - i])))
        except ValueError:
            pass
    ...
    if(bernoulli.rvs(transitionProba) == 1):
        ...
        diff = end - start
        return F, boundary_edges, interior_edges, np.nan, np.nan, np.nan, np.nan
    else:
